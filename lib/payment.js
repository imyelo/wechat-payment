...
// reference to payment api document 3.5
// beta
Payment.prototype.validateNativePayGetPackage = function (params) {
  ...
  _.each(params, function (val, key) {
    if ([
      ...
      'openid',
      'issubscribe'
    ].indexOf(key = key.toLowerCase()) > -1) {
      result[key] = val;
    }
  });
  return params['AppSignature'] === this.getPaySign(_.assign(result, {appkey: true}));
};

// reference to payment api document 3.6
// beta
...
Payment.prototype.validateNotifySign = function (params) {
  var noEmpty = _.filter(params, function (val) {
    return !!val && val !== 'sign';
  });
